...
import {
  ...
  Waves, Building2, Camera, Settings, Pencil, Trash2, TreePine, MountainSnow, Landmark, Bike, ClipboardCheck, Loader2, Play, Images, Info as InfoIcon
} from "lucide-react";
...
const T = {
  ...
  usGear: { uk: "Спорядження", en: "What to bring", de: "Ausrüstung", ru: "Снаряжение" },
  usTransport: { uk: "Транспорт", en: "Transport", de: "Verkehr", ru: "Транспорт" },
  usD1: { uk: "Deutschland-Ticket — 63 € на місяць, діє на всі регіональні поїзди, автобуси й трамваї по всій Німеччині. Якщо їздите з нами частіше ніж раз на місяць, окупається одразу.", en: "Deutschland-Ticket — €63 a month, valid on all regional trains, buses and trams across Germany. Pays for itself if you join more than once a month.", de: "Deutschland-Ticket — 63 € im Monat, gültig in allen Regionalzügen, Bussen und Straßenbahnen.", ru: "Deutschland-Ticket — 63 € в месяц, действует на все региональные поезда, автобусы и трамваи по всей Германии." },
  usD2: { uk: "Bayern-Ticket — 34 € на день на компанію до 5 осіб по Баварії. Вигідний, коли їдете разом і лише на один день.", en: "Bayern-Ticket — €34 a day for up to 5 people across Bavaria. Worth it for a one-day trip together.", de: "Bayern-Ticket — 34 € pro Tag für bis zu 5 Personen in Bayern.", ru: "Bayern-Ticket — 34 € в день на компанию до 5 человек по Баварии." },
  usG1: { uk: "Взуття з протектором — головне. Решта залежить від маршруту: воду беріть завжди, дощовик у горах теж завжди.", en: "Shoes with grip matter most. The rest depends on the route: always water, and always a rain jacket in the mountains.", de: "Schuhe mit Profil sind das Wichtigste. Wasser immer, Regenjacke in den Bergen immer.", ru: "Обувь с протектором — главное. Вода всегда, дождевик в горах тоже всегда." },
  usG2: { uk: "Готівка на квитки й кафе: у гірських кафе картки часто не приймають.", en: "Cash for tickets and cafés: mountain cafés often do not take cards.", de: "Bargeld für Tickets und Hütten: dort wird oft keine Karte akzeptiert.", ru: "Наличные на билеты и кафе: в горных кафе карты часто не принимают." },
  usT1: { uk: "Розклад перевіряйте в день поїздки — регіональні поїзди в Баварії скасовують нерідко.", en: "Check the schedule on the day — regional trains in Bavaria are cancelled fairly often.", de: "Fahrplan am Tag prüfen — Regionalzüge fallen in Bayern öfter aus.", ru: "Расписание проверяйте в день поездки — региональные поезда в Баварии отменяют нередко." },
  usT2: { uk: "Приходьте на місце зустрічі за 10 хвилин: група не чекає, поїзд теж.", en: "Arrive at the meeting point 10 minutes early: the group will not wait, and neither will the train.", de: "10 Minuten früher am Treffpunkt sein: Gruppe und Zug warten nicht.", ru: "Приходите на место встречи за 10 минут: группа не ждёт, поезд тоже." },
...
function DriveGallery({ folderUrl, limit }) {
  ...
  const cur = open != null ? files[open] : null;
  const isVideo = (f) => String(f.mimeType || "").startsWith("video/");
    ...
    <>
      ...
      {cur && (
        <div onClick={() => setOpen(null)}
          style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.92)", zIndex: 60, display: "flex", alignItems: "center", justifyContent: "center", padding: 12 }}>
          {isVideo(cur) ? (
            <iframe title="video" src={drivePreview(cur.id)} allow="autoplay"
              style={{ width: "100%", maxWidth: 900, aspectRatio: "16/9", border: "none", borderRadius: 12 }} />
          ) : (
      ...
      <div style={{ maxWidth: 440, margin: "0 auto", background: C.page, minHeight: "100vh", position: "relative", overflow: "hidden" }}>
        ...
        {trip ? (
          ...
          <div style={{ padding: "0 16px 30px" }}>
            ...
            {tab === "useful" && (
              <div style={{ background: C.card, borderRadius: 18, padding: 16, marginBottom: 16 }}>
                <h2 style={{ margin: "0 0 14px", fontSize: 15, fontWeight: 800, color: C.ink }}>{t("usTitle")}</h2>
                {[
                  [<Train size={16} />, t("usTickets"), [t("usD1"), t("usD2")]],
                  [<Footprints size={16} />, t("usGear"), [t("usG1"), t("usG2")]],
                  [<Navigation size={16} />, t("usTransport"), [t("usT1"), t("usT2")]],
                ].map(([icon, title, items], i) => (
                  <div key={i} style={{ marginBottom: i === 2 ? 0 : 16 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8, color: C.greenDark }}>
                      {icon}
                      <span style={{ fontSize: 13, fontWeight: 800, color: C.ink }}>{title}</span>
                    </div>
                    {items.map((x, j) => (
                      <p key={j} style={{ margin: "0 0 8px", fontSize: 13, color: C.inkSoft, lineHeight: 1.55 }}>{x}</p>
                    ))}
                  </div>
                ))}
              </div>
