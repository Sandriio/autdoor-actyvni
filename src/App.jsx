...
import {
  ...
  Waves, Building2, Camera, Settings, Pencil, Trash2, TreePine, Landmark, Bike, ClipboardCheck, Loader2, Play, Lock, Info as InfoIcon
} from "lucide-react";
...
async function pushSend(pin, title, body, url, tag) {
  const r = await fetch("/api/push", {
    ...
    body: JSON.stringify({ pin, title, body, url: url || "/", tag: tag || "tropa" }),
  });
...
// Чернетки не повинні покидати базу для глядача. Раніше застосунок
// завантажував повний вміст усіх поїздок у кожен телефон і вже потім
// вирішував, що показати — тобто назву чернетки однаково можна було
// прочитати за публічною адресою бази. Тепер база сама не віддає
// чернеток глядачеві (правило в supabase-v123.sql), а замість них дає
// лише колір картки — через trip_teasers.
async function sbTripTeasers() {
  const r = await fetch(`${SUPABASE_URL}/rest/v1/rpc/trip_teasers`, {
    method: "POST", headers: sbHeaders(), body: "{}",
  });
  if (!r.ok) return [];   // функції ще немає — просто без заглушок
  const rows = await r.json();
  return Array.isArray(rows) ? rows : [];
}

// Організатор отримує все, чернетки теж — але лише за PIN.
async function sbAdminTrips(pin) {
  const r = await fetch(`${SUPABASE_URL}/rest/v1/rpc/admin_trips`, {
    method: "POST", headers: sbHeaders(), body: JSON.stringify({ p_pin: pin }),
  });
  if (!r.ok) throw new Error("admin " + r.status);
  const rows = await r.json();
  return rows.map((x) => x.trip);
}

// Єдина точка читання: сама вирішує, що кому показати.
//
// Для глядача чернетки з повного списку прибираються ЗАВЖДИ, а з них
// лишається тільки колір. Це важливо, поки SQL ще не виконано: тоді
// база ще віддає чернетки повністю, і без цього кроку їхні назви
// потрапили б у стан застосунку. Звідки б не прийшла заглушка — з
// правила бази чи з цього фільтра, — у стані лежить лише колір.
async function sbTripsFor(pin) {
  if (pin) {
    // Функції для організатора ще немає — не залишаємо його без поїздок.
    try { return { trips: await sbAdminTrips(pin), teasers: [] }; }
    catch { /* падаємо на звичайне читання нижче */ }
  }
  const all = await sbLoadTrips();
  const leaked = all.filter(isDraft);
  const trips = all.filter((x) => !isDraft(x));
  const fromDb = leaked.length ? [] : await sbTripTeasers().catch(() => []);
  const teasers = [
    ...leaked.map((x) => x.heroGradient),
    ...fromDb.map((x) => x.hero_gradient),
  ].map((g, i) => ({ id: `draft-${i}`, draft: true, heroGradient: g || "" }));
  return { trips: pin ? all : trips, teasers: pin ? [] : teasers };
}
...
const T = {
  ...
  draftBadge: { uk: "В розробці · бачиш лише ти", en: "Draft · only you see it", de: "Entwurf · nur für dich sichtbar", ru: "В разработке · видишь только ты" },
...
// Поїздка в розробці. Глядач бачить на її місці лише кольорову заглушку —
// ні назви, ні дати, ні маршруту. Організатор бачить усе, щоб готувати.
// Перевіряємо саме === true: у старих поїздках поля немає взагалі, і вони
// мають лишатися видимими, як і були.
const isDraft = (trip) => Boolean(trip) && trip.draft === true;
...
const TRIPS = [
  {
    ...
    contact: { name: "Андрій", role: "Організатор поїздки", telegram: "@tropa_club", phone: "+49 155 617 12359" },
...
// ── Власні іконки місць ──────────────────────────────────────────────
// У бібліотеці значків немає ні ущелини, ні водоспаду, ні долини, ні
// ріки. Раніше на їхньому місці стояли чужі значки: ущелина малювалась
// горою (тією самою, що й «Гори»), долина — засніженою горою, ріка —
// краплями, водоспад — дощовою хмарою.
//
// Ці чотири намальовані за правилами решти значків: сітка 24×24, без
// заливки, та сама товщина лінії й заокруглені кінці. Колір береться
// від батьківського елемента (currentColor), як і в інших значків, —
// тож на картці вони так само білі.
//
// Кожну звірено в справжньому розмірі, 26 px білим на кольоровій
// картці, поруч з усіма іншими типами: жодна не повторює іншу.
//  • Ущелина — дві скелі з розколиною, вода лише внизу.
//  • Водоспад і ріка — за малюнками організатора (сплеск і водоворот),
//    перенесеними на ту саму сітку й товщину лінії. Див. нижче.
//  • Долина — плавні пагорби з сонцем між ними. Саме плавні лінії
//    замість гострих вершин і відрізняють її від гір.
//
// Водоворот (ріка). Спіраль не обведена вручну, а розрахована: оригінал
// виявився майже ідеальною подвійною архімедовою спіраллю (відхилення
// 1–1,5 px на радіусі 224), тож тут та сама формула й той самий напрям
// закручування — накладання на оригінал збігається на 73 %, дзеркальне
// лише на 16 %. Витків 1,1 замість 1,95: у малюнку лінія займає 3,5 %
// діаметра, у значках — 7,5 %, і на 26 px повні витки зливались у
// пляму. Сімнадцять бульбашок оригіналу зведено до чотирьох: кожне
// скупчення (ліве, найбільше, праве, верхнє, нижнє) лягло у свій
// найближчий вільний кут, розмір збережено за старшинством.
//
// Сплеск (водоспад). Три кола в оригіналі сплюснуті втричі, тож спереду
// між ними лише 0,9 клітинки — менше за товщину лінії, і на 26 px вони
// злипались у суцільну смугу. Лишилось одне коло, а роль внутрішнього
// бере розширена основа струменя, як і в оригіналі. Струмінь трохи
// ширший за оригінальний, щоб усередині лишився просвіт, і нижчий, щоб
// над ним вмістились бризки. Бульбашки розведено дугою: в оригіналі
// вони майже торкаються верху струменя, і на 26 px злились би з ним.
//
// Найменший просвіт між лініями в обох — 1,1 клітинки й більше
// (перевірено обчисленням), відступ від краю — не менше 1.
const PLACE_GLYPHS = {
  gorge: (
    <>
      <path d="M3 21V3h5l-1.2 4 2 3.2-1.4 4L10 19" />
      <path d="M21 21V3h-5l1.2 4-2 3.2 1.4 4L14 19" />
      <path d="M10 19c.7.6 1.3.6 2 0s1.3-.6 2 0" />
    </>
  ),
  waterfall: (
    <>
      <path d="M8 17C9.15 17 10.55 14.93 10.55 12.4V7.4A1.45 1.45 0 0 1 13.45 7.4V12.4C13.45 14.93 14.85 17 16 17Q12 18.1 8 17Z" />
      <path d="M2 17.6A10 3.35 0 0 1 10.28 14.3M13.72 14.3A10 3.35 0 0 1 22 17.6M22 17.6A10 3.35 0 0 1 12 20.95A10 3.35 0 0 1 2 17.6" />
      <circle cx="6.9" cy="4.4" r="1.35" />
      <circle cx="16.8" cy="4.7" r=".9" />
      <circle cx="11.7" cy="2.45" r=".5" />
    </>
  ),
  river: (
    <>
      <path d="M13.26 11.01C13.67 11.17 14.07 11.53 14.28 12.06C14.49 12.6 14.51 13.29 14.23 13.95C13.95 14.61 13.37 15.24 12.58 15.59C11.79 15.95 10.79 16.03 9.81 15.73C8.84 15.42 7.91 14.72 7.3 13.71C6.69 12.71 6.42 11.4 6.65 10.09C6.87 8.79 7.6 7.48 8.74 6.54C9.89 5.59 11.44 5.02 13.08 5.04C14.71 5.07 16.42 5.69 17.77 6.87C19.12 8.05 20.1 9.78 20.39 11.71" />
      <path d="M10.74 12.99C10.33 12.83 9.93 12.47 9.72 11.94C9.51 11.4 9.49 10.71 9.77 10.05C10.05 9.39 10.63 8.76 11.42 8.41C12.21 8.05 13.21 7.97 14.19 8.27C15.16 8.58 16.09 9.28 16.7 10.29C17.31 11.29 17.58 12.6 17.35 13.91C17.13 15.21 16.4 16.52 15.26 17.46C14.11 18.41 12.56 18.98 10.92 18.96C9.29 18.93 7.58 18.31 6.23 17.13C4.88 15.95 3.9 14.22 3.61 12.29" />
      <circle cx="4.15" cy="4.93" r="1.5" />
      <circle cx="19.48" cy="18.73" r="1" />
      <circle cx="19.48" cy="3.69" r=".6" />
      <circle cx="4.52" cy="20.31" r=".6" />
    </>
  ),
  valley: (
    <>
      <path d="M2 16c1.5-3 3-5 4.5-5s3 2 5.5 6c2.5-4 4-6 5.5-6s3 2 4.5 5" />
      <path d="M2 20h20" />
...
function PlaceGlyph({ kind, size, strokeWidth }) {
  const n = size || 18;
  return (
...
function placeIcon(type, size) {
  ...
  return {
    ...
    gorge: <PlaceGlyph kind="gorge" size={n} />,
    mountain: <Mountain size={n} strokeWidth={1.8} />,
    forest: <TreePine size={n} strokeWidth={1.8} />,
    valley: <PlaceGlyph kind="valley" size={n} />,
    river: <PlaceGlyph kind="river" size={n} />,
    museum: <Landmark size={n} strokeWidth={1.8} />,
    waterfall: <PlaceGlyph kind="waterfall" size={n} />,
    bike: <Bike size={n} strokeWidth={1.8} />,
  }[type] || <Mountain size={n} strokeWidth={1.8} />;
}

// ── Значок альбому: діафрагма ───────────────────────────────────────
// Вісім однакових пелюсток, кожна повернута на 45°. Фігура обведена з
// нашого фірмового знака, тому одного контуру досить — решту робить
// поворот, і значок важить кілька рядків замість кількох кілобайтів.
//
// Проміжки між пелюстками свідомо ширші, ніж у вихідному малюнку. На
// 28 пікселях тонкі щілини зникають під час згладжування, і значок
// перетворюється на суцільну пляму — перевірено промальовуванням на
// 24, 34 і 48 пікселях. Ширші проміжки читаються на всіх розмірах.
const APERTURE_BLADE = "M22.62,10.68 21.94,11.47 20.76,12.54 19.39,13.49 18.06,14.15 16.59,14.66 15.24,14.94 14.14,15.05 12.38,15.01 12.81,15.38 13.79,16.05 14.87,16.61 16.17,17.07 17.57,17.38 18.91,17.50 20.32,17.42 21.32,17.25 21.41,17.17 22.03,15.84 22.30,15.06 22.57,13.89 22.73,12.54 22.74,11.67Z";
const APERTURE_BLADES = [0, 1, 2, 3, 4, 5, 6, 7];
function ApertureIcon({ size, color }) {
  const n = size || 28;
  return (
    <svg width={n} height={n} viewBox="0 0 24 24" fill={color || "currentColor"} aria-hidden="true" style={{ display: "block" }}>
      {APERTURE_BLADES.map((i) => (
        <path key={i} d={APERTURE_BLADE} transform={`rotate(${i * 45} 12 12)`} />
      ))}
    </svg>
  );
}
...
function DriveGallery({ folderUrl, limit, albumId, isAdmin, adminPin }) {
  ...
  // цього телефон бере заголовок сторінки; назву застосунку задаємо самі,
  // щоб у шторці стояла вона, а не те, що вирішить браузер.
  useEffect(() => {
    ...
    try {
      navigator.mediaSession.metadata = new window.MediaMetadata({
        title: String(cur.name || "").replace(/\.[a-z0-9]+$/i, "") || t("appName"),
        artist: t("appName"),
        album: t("appName"),
      });
    } catch { /* підпис — дрібниця, через неї не варто ламати перегляд */ }
  }, [cur && cur.id]);
        ...
        <div onClick={() => setOpen(null)}
          ...
          {open > 0 && (
            <button onClick={(e) => { e.stopPropagation(); setOpen(open - 1); }}
              style={{ position: "absolute", left: 8, top: "50%", transform: "translateY(-50%)", width: 40, height: 40, borderRadius: "50%", border: "none", background: "rgba(255,255,255,0.85)", color: C.ink, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
...
// Вісім окремих плакатів. У списку кожен показується зменшеним, натиск
// відкриває його на весь екран зі збільшенням: текст на плакатах дрібний
// і на телефоні без збільшення не читається.
//
...
function UsefulSheetViewer({ sheet, title, onClose, onPrev, onNext }) {
  // Збільшення кнопками, а не пальцями: жести всередині накладеного вікна
  // працюють по-різному на iPhone і на Android, а кнопки «+» і «−»
  // поводяться однаково скрізь і зрозумілі без пояснень.
  const [z, setZ] = useState(0);
  useEffect(() => { setZ(0); }, [sheet]);
  const zoom = USEFUL_ZOOMS[z];

  const btn = (label, onClick, enabled, radius) => (
    <button onClick={(e) => { e.stopPropagation(); onClick(); }} disabled={!enabled}
      aria-label={label === "+" ? "Збільшити" : "Зменшити"}
      style={{
        width: 40, height: 40, border: "none", background: "#fff", color: enabled ? C.green : C.faint,
        fontSize: 22, fontWeight: 700, lineHeight: 1, cursor: enabled ? "pointer" : "default",
        fontFamily: "inherit", display: "flex", alignItems: "center", justifyContent: "center",
        borderRadius: radius, padding: 0,
      }}>{label}</button>
  );
  ...
  return (
    <div onClick={onClose}
      style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.93)", zIndex: 60, display: "flex", flexDirection: "column" }}>
      <div style={{ flex: 1, overflow: "auto", WebkitOverflowScrolling: "touch", padding: "calc(58px + env(safe-area-inset-top)) 10px calc(16px + env(safe-area-inset-bottom))" }}>
        <img src={sheet} alt={title} onClick={(e) => e.stopPropagation()}
          style={{ width: `${zoom * 100}%`, maxWidth: "none", display: "block", margin: "0 auto", borderRadius: zoom === 1 ? 12 : 0 }} />
      </div>
      <button onClick={(e) => { e.stopPropagation(); onClose(); }}
        style={{ position: "absolute", top: "calc(14px + env(safe-area-inset-top))", right: 14, width: 38, height: 38, borderRadius: "50%", border: "none", background: "rgba(255,255,255,0.9)", color: C.ink, fontSize: 20, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>×</button>
      <div style={{ position: "absolute", top: "calc(14px + env(safe-area-inset-top))", left: 14, display: "flex", boxShadow: "0 2px 8px rgba(0,0,0,0.3)", borderRadius: 10, overflow: "hidden" }}>
        {btn("−", () => setZ((v) => Math.max(0, v - 1)), z > 0, "10px 0 0 10px")}
        <div style={{ width: 1, background: C.line }} />
        {btn("+", () => setZ((v) => Math.min(USEFUL_ZOOMS.length - 1, v + 1)), z < USEFUL_ZOOMS.length - 1, "0 10px 10px 0")}
      </div>
      {onPrev && (
        <button onClick={(e) => { e.stopPropagation(); onPrev(); }}
          style={{ position: "absolute", left: 8, bottom: "calc(18px + env(safe-area-inset-bottom))", width: 42, height: 42, borderRadius: "50%", border: "none", background: "rgba(255,255,255,0.85)", color: C.ink, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <ChevronRight size={20} style={{ transform: "rotate(180deg)" }} />
        </button>
      )}
      {onNext && (
        <button onClick={(e) => { e.stopPropagation(); onNext(); }}
          style={{ position: "absolute", right: 8, bottom: "calc(18px + env(safe-area-inset-bottom))", width: 42, height: 42, borderRadius: "50%", border: "none", background: "rgba(255,255,255,0.85)", color: C.ink, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <ChevronRight size={20} />
        </button>
      )}
    </div>
...
function TripCard({ trip, onClick, isAdmin, onSetStatus, onSetPostponedDate, onEdit, counts }) {
  // Глядачеві — лише колір і напис. Без назви, дати й будь-яких деталей,
  // і картка нікуди не веде: відкрити чернетку глядач не може.
  if (isDraft(trip) && !isAdmin) {
    return (
      <div style={{
        width: "100%", borderRadius: 20, overflow: "hidden", marginBottom: 16,
        boxShadow: "0 2px 14px rgba(60,79,44,0.08)", background: trip.heroGradient || C.green,
      }}>
        {/* Легке затемнення знизу: заглушка може бути будь-якого кольору,
            і на світлих градієнтах білий напис без нього губиться. */}
        <div style={{
          minHeight: 150, padding: "22px 20px", display: "flex", flexDirection: "column", justifyContent: "center", gap: 12,
          background: "linear-gradient(180deg, rgba(0,0,0,0.04) 0%, rgba(0,0,0,0.26) 100%)",
        }}>
          <span style={{ width: 42, height: 42, borderRadius: 13, background: "rgba(255,255,255,0.22)", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <Lock size={20} color="#fff" />
          </span>
          <div>
            <div style={{ fontSize: 19, fontWeight: 800, color: "#fff", letterSpacing: -0.2, lineHeight: 1.2 }}>{t("draftTitle")}</div>
            <div style={{ fontSize: 13.5, color: "rgba(255,255,255,0.9)", marginTop: 4, lineHeight: 1.45 }}>{t("draftSub")}</div>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div style={{
      ...
      <div onClick={onClick} style={{ cursor: "pointer" }}>
        {isAdmin && isDraft(trip) && (
          <div style={{
            display: "flex", alignItems: "center", justifyContent: "center", gap: 6,
            background: "#2b3a1f", color: "#fde446", padding: "7px 12px",
            fontSize: 10.5, fontWeight: 800, letterSpacing: 0.6, textTransform: "uppercase",
          }}>
            <Lock size={12} /> {t("draftBadge")}
          </div>
        )}
...
// Файли лишаються на Диску: там їх зручно вивалювати з телефона цілими
// пачками, і вони не з'їдають наше сховище. Застосунок лише питає, що
// в теці лежить, і показує це своїми руками.
//
// Ключ обмежений доменом і одним сервісом — з чужого сайту він марний,
// тому лежить у коді так само, як ключ Supabase.
const GDRIVE_KEY = "AIzaSyDoAE6i_3BQ-tJjxwUVkOFk_Dn4IA7FcI4";
...
const BLANK_TRIP = () => ({
  ...
  date: "",
  dateLabel: "",
  // Нова поїздка одразу в розробці: публікуєш свідомо, коли вона готова.
  draft: true,
  difficulty: "Легкий",
  difficultyNote: "",
  distanceKm: 0,
    ...
    <div style={{ paddingBottom: 100 }}>
      ...
      <div style={{ padding: "18px 16px 0", marginTop: -12, background: C.page, borderRadius: "20px 20px 0 0" }}>
        ...
        <div style={card}>
          ...
          {/* Один перемикач на поле draft. Раніше тут стояли два — старий
              червоний і цей, — і обидва міняли те саме поле. */}
          <Field label="Стан підготовки">
          <button type="button" onClick={() => set({ draft: !isDraft(t) })}
            style={{
              display: "flex", alignItems: "center", gap: 12, width: "100%", textAlign: "left",
              background: isDraft(t) ? "#fff4d6" : "#fff", border: `1.5px solid ${isDraft(t) ? "#e2b43c" : C.line}`,
              borderRadius: 14, padding: "13px 14px", marginBottom: 14, cursor: "pointer", fontFamily: "inherit",
            }}>
            <span style={{ width: 38, height: 38, borderRadius: 11, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center",
              background: isDraft(t) ? "#e2b43c" : C.greenSoft, color: isDraft(t) ? "#fff" : C.greenDark }}>
              <Lock size={18} />
            </span>
            <span style={{ flex: 1, minWidth: 0 }}>
              <span style={{ display: "block", fontSize: 14, fontWeight: 800, color: C.ink }}>
                {isDraft(t) ? "Поїздка в розробці" : "Поїздка опублікована"}
              </span>
              <span style={{ display: "block", fontSize: 12, color: C.muted, marginTop: 2, lineHeight: 1.45 }}>
                {isDraft(t)
                  ? "Глядачі бачать лише кольорову картку без назви. Натисни, коли поїздка буде готова."
                  : "Усі бачать поїздку повністю. Натисни, щоб знову сховати."}
              </span>
            </span>
            <span style={{ width: 44, height: 26, borderRadius: 13, flexShrink: 0, position: "relative",
              background: isDraft(t) ? "#e2b43c" : C.green, transition: "background .15s" }}>
              <span style={{ position: "absolute", top: 3, left: isDraft(t) ? 21 : 3, width: 20, height: 20,
...
export default function App() {
  ...
  // Заглушки чернеток для глядача: лише колір, без жодних даних поїздки.
  // Окремо від trips свідомо — уся решта логіки (стани, дати, сортування)
  // чекає повної поїздки і на заглушці зламалася б.
  const [teasers, setTeasers] = useState([]);
  // Який розділ головного екрана відкритий: поїздки, фотоархів, корисне.
  ...
  const reloadTrips = () => {
    ...
    setLoadError(false);
    sbTripsFor(adminPin)
      .then((r) => { setTrips(r.trips); setTeasers(r.teasers); setLoadingTrips(false); })
  ...
  useEffect(() => {
    ...
    // Залежить від PIN: увійшов організатор — перечитуємо з чернетками,
    // вийшов — перечитуємо без них. Інакше після входу чернеток не було
    // б видно, а після виходу вони лишалися б на екрані.
    if (!sbConfigured()) return;
    let cancelled = false;
    sbTripsFor(adminPin)
      .then((r) => { if (!cancelled) { setTrips(r.trips); setTeasers(r.teasers); setLoadingTrips(false); } })
      ...
      <div style={{ maxWidth: 440, margin: "0 auto", background: C.page, minHeight: "100vh", position: "relative", overflow: "hidden" }}>
        ...
        {trip && !(isDraft(trip) && !isAdmin) ? (
          ...
          <div style={{ padding: "0 16px 30px" }}>
            ...
            {/* Три розділи. Поїздки — те, задля чого застосунок відкривають
                щодня; фотоархів і корисне живуть окремо, щоб не відсувати
                список поїздок униз. */}
            <div style={{ display: "flex", gap: 6, marginBottom: 16, background: "rgba(255,255,255,0.08)", borderRadius: 14, padding: 4 }}>
              {[["trips", t("tabTrips")], ["photos", t("tabPhotos")], ["useful", t("tabUseful")]].map(([k, label]) => (
                <button key={k} onClick={() => setTab(k)}
                  style={{
                    flex: 1, border: "none", borderRadius: 11, padding: "9px 6px",
                    background: tab === k ? "rgba(253,228,70,0.92)" : "transparent",
                    color: tab === k ? C.greenDark : "rgba(255,255,255,0.78)",
            ...
            {tab === "trips" && isAdmin && <div><RedList pin={adminPin} /></div>}
            {tab === "trips" && isAdmin && trips.length > 0 && (
            ...
            {!loadingTrips && !loadError && upcoming.length === 0 && teasers.length === 0 && <p style={{ fontSize: 13, color: "rgba(255,255,255,0.6)", margin: "0 4px 14px" }}>{t("noUpcoming")}</p>}
            ...
            {!loadingTrips && !loadError && teasers.map((d) => (
              <TripCard key={d.id} trip={d} isAdmin={false} counts={counts} onClick={() => {}} />
            ))}
            {upcoming.map((t) => (
            ...
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", marginTop: 26, gap: 14 }}>
              {null}
              {isAdmin ? (
                <button onClick={() => { setIsAdmin(false); setAdminPin(""); }} style={{ background: "none", border: "1px solid rgba(255,255,255,0.3)", color: "rgba(255,255,255,0.7)", borderRadius: 20, padding: "7px 14px", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
                  {t("exitOrganizer")}
                </button>
              ) : (
